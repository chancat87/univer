...
var AccountingPanel = (props) => {
  ...
  const [decimal, setDecimal] = (0, import_react2.useState)(() => getDecimalFromPattern(defaultPattern || "", 2));
  const userHabitCurrency = (0, import_react2.useContext)(UserHabitCurrencyContext);
  const [suffix, setSuffix] = (0, import_react2.useState)(() => getCurrencyType(defaultPattern) || userHabitCurrency[0]);
  const options = (0, import_react2.useMemo)(() => userHabitCurrency.map((key3) => ({ label: key3, value: key3 })), []);
...
var CurrencyPanel = (props) => {
  ...
  const [suffix, setSuffix] = (0, import_react3.useState)(() => getCurrencyType(props.defaultPattern) || userHabitCurrency[0]);
  const [decimal, setDecimal] = (0, import_react3.useState)(() => getDecimalFromPattern(props.defaultPattern || "", 2));
  const [pattern, setPattern] = (0, import_react3.useState)(() => {
    var _a;
  ...
  const negativeOptions = (0, import_react3.useMemo)(() => getCurrencyFormatOptions(suffix), [suffix]);
  const options = (0, import_react3.useMemo)(() => userHabitCurrency.map((key3) => ({ label: key3, value: key3 })), [userHabitCurrency]);
...
function CustomFormat(props) {
  ...
  const localeService = useDependency(LocaleService);
  const [pattern, setPattern] = (0, import_react4.useState)(defaultPattern);
...
function DatePanel(props) {
  ...
  const options = (0, import_react5.useMemo)(getDateFormatOptions, []);
  const localeService = useDependency(LocaleService);
  const [suffix, setSuffix] = (0, import_react5.useState)(() => {
    if (defaultPattern) {
...
function ThousandthPercentilePanel(props) {
  ...
  const localeService = useDependency(LocaleService);
  const options = (0, import_react7.useMemo)(getNumberFormatOptions, []);
  const [decimal, setDecimal] = (0, import_react7.useState)(() => getDecimalFromPattern(props.defaultPattern || "", 0));
  ...
  const pattern = (0, import_react7.useMemo)(() => setPatternDecimal(suffix, Number(decimal || 0)), [suffix, decimal]);
  const isInputDisable = (0, import_react7.useMemo)(() => !isPatternHasDecimal(suffix), [suffix]);
...
var useCurrencyOptions = (onOptionChange) => {
  const userHabitController = useDependency(UserHabitController);
  const [options, setOptions] = (0, import_react8.useState)(currencySymbols);
...
var useNextTick = () => {
  const effectList = (0, import_react9.useRef)([]);
  const [value, dispatch] = (0, import_react9.useState)({});
...
var SheetNumfmtPanel = (props) => {
  ...
  const localeService = useDependency(LocaleService);
  const getCurrentPattern = (0, import_react10.useRef)(() => "");
  const nextTick = useNextTick();
  const typeOptions = (0, import_react10.useMemo)(
    () => [
  ...
  const [type, setType] = (0, import_react10.useState)(findDefaultType);
  const [key3, setKey] = (0, import_react10.useState)(() => `${row}_${col}`);
  const { mark, userHabitCurrency } = useCurrencyOptions(() => setKey(`${row}_${col}_userCurrency'`));
  const BusinessComponent = (0, import_react10.useMemo)(() => {
    var _a;
  ...
  const handleChange = (0, import_react10.useCallback)((v) => {
    props.onChange({ type: "change", value: v });
  }, []);
  const handleActionChange = (0, import_react10.useCallback)((action) => {
    ...
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ (0, import_jsx_runtime7.jsxs)("div", { children: [
          ...
          /* @__PURE__ */ (0, import_jsx_runtime7.jsx)("div", { children: BusinessComponent && /* @__PURE__ */ (0, import_jsx_runtime7.jsx)(UserHabitCurrencyContext.Provider, { value: userHabitCurrency, children: /* @__PURE__ */ (0, import_react11.createElement)(BusinessComponent, { ...subProps, key: key3 }) }) })
        ] }),
...
function Options() {
  ...
  const menuOptions = (0, import_react12.useMemo)(() => {
