...
function ThousandthPercentilePanel(props) {
  ...
  const localeService = useDependency(LocaleService);
  const options = (0, import_react9.useMemo)(getNumberFormatOptions, []);
  const [decimal, setDecimal] = (0, import_react9.useState)(() => getDecimalFromPattern(props.defaultPattern || "", 0));
  const [suffix, setSuffix] = (0, import_react9.useState)(() => {
  ...
  const pattern = (0, import_react9.useMemo)(() => setPatternDecimal(suffix, Number(decimal || 0)), [suffix, decimal]);
  const isInputDisable = (0, import_react9.useMemo)(() => !isPatternHasDecimal(suffix), [suffix]);
